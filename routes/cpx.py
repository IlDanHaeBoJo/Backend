...
async def get_single_cpx_details(
    ...
    # 응답 데이터 생성
    response_data = CpxFullDetailsResponse.model_validate(cpx_result).model_dump()
...
async def get_all_cpx_results_for_admin(
    ...
    # 각 결과에 마크다운 피드백 추가
    response_list = []
    for cpx in cpx_results:
        response_data = CpxFullDetailsResponse.model_validate(cpx).model_dump()
        
        # 마크다운 피드백은 이미 DB에 저장되어 있으므로 별도 처리 불필요
