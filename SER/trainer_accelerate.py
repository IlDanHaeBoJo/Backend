...
def train_model_accelerate(accelerator, model, train_loader, val_loader, num_epochs=3, learning_rate=3e-5):
    ...
    class_weights = torch.tensor([2.0, 1.5, 0.7, 0.5], dtype=torch.float32).to(accelerator.device)
    ...
    with torch.no_grad():            
        for eval_step, batch in enumerate(dataloader):
            ...
            if loss is None:
                ...
                loss = loss_fct(outputs['emotion_logits'].view(-1, model.config.num_labels), 
                               batch['labels'].view(-1))
    ...
    try:
        ...
        if accelerator.is_main_process:
            ...
            wandb.init(
                ...
                name="adv lambda 0.5 (spk id normal) 20 epoch Adversary 4 Class Classification Large Dataset Balancing (Multi-GPU)",
                config={
        ...
        try:
            ...
            if accelerator.is_main_process:
                ...
                model_save_path = os.path.join(output_dir, "Adversary_class4_v3_LargeDataset_MultiGPU_batch8_real_adv_lambda_0.5_20_epoch_spk_id_normal")
